...
class SearchCustomObjectRecordsRequestTypedDict(TypedDict):
...
class SearchCustomObjectRecordsRequest(BaseModel):
...
class SearchCustomObjectRecordsResponse(BaseModel):
    next: Callable[[], Optional[SearchCustomObjectRecordsResponse]]
